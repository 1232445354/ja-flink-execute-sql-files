...
create table adsb_exchange_aircraft_list_kafka(
      ...
      'scan.startup.timestamp-millis' = '1720441228000',
      'format' = 'json',
...
select
    ...
    coalesce(if(t='',cast(null as string),t),c.icao_type)          as flightType,       -- 飞机型号
  ...
  cast(null as string)                                           as flightStatus,    -- 飞机状态： 已启程
...
select
    ...
    coalesce(t7.country_code,t6.country_code,t5.country_code,t4.country_code) as countryCode
...
select
    ...
    c.c_name as countryName						    , -- 飞机所属的国家
    case
        when countryCode in('IN','US','JP','AU') and isMilitary=1 then 'ENEMY' -- 敌
        when countryCode='CN' and isMilitary=1  then 'OUR_SIDE' -- 我
        when countryCode='CN' and isMilitary=0  then 'FRIENDLY_SIDE' -- 友
...
from tmp_dws_aircraft_combine_list_rt_04 a
         ...
         left join dim_country_code_name_info FOR SYSTEM_TIME AS OF a.proctime as c
                   on a.countryCode = c.country_code2
                       and 'COMMON' = c.source;
