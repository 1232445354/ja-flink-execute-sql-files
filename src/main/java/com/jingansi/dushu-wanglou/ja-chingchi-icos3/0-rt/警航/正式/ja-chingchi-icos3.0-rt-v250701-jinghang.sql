...
-- version:ja-chingchi-icos3.0-rt-v250701-jinghang 密集数据接入
--********************************************************************--
...
create table dwd_device_attr_info (
                                      ...
                                      longitude                 DECIMAL(30,18)      comment '经度—高德坐标系、火星坐标系',
                                      latitude                  DECIMAL(30,18)      comment '纬度—高德坐标系、火星坐标系',
...
select
    ...
    cast(JSON_VALUE(properties,'$.longitude') as double)       as longitude,
    cast(JSON_VALUE(properties,'$.latitude') as double)        as latitude,
    cast(JSON_VALUE(properties,'$.attitude_head') as double)   as attitude_head, -- 无人机机头朝向
    cast(JSON_VALUE(properties,'$.gimbal_head') as double)     as gimbal_head, -- 无人机云台朝向
    cast(JSON_VALUE(properties,'$.altitude') as double)        as altitude, -- 海拔高度
    cast(JSON_VALUE(properties,'$.height') as double)          as height, -- 跟海拔差不多的一个高度，用于回溯轨迹3维
    device_name_join                                           as device_name, -- 设备名称
