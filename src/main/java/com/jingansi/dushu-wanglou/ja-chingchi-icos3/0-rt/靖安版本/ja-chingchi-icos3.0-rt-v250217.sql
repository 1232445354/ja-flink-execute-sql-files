...
-- version:ja-chingchi-icos3.0-rt-v250217 城市态势版本
--********************************************************************--

set
''pipeline.name'' = ''ja-chingchi-icos3.0-rt'';
...
600000'';
SET
''state.checkpoints.dir'' = ''s3://flink/flink-checkpoints/ja-chingchi-icos3.0-rt'';
...
(
    ...
    -- 手动拍照截图数据
    `data`      row( pictureUrl string, -- 拍照数据上报-图片url
        width int ,                     -- 拍照数据上报-宽度
        height int                      -- 拍照数据上报-高度
        ),

    message     row( tid string,        -- 当前请求的事务唯一ID
...
(
    device_id                string comment '' 设备id:望楼id,
    雷达ID,
    可见光红外id,
    震动器id '',
...
select coalesce(productKey, message.productKey)   as product_key,       -- message_product_key
...
where `method` = ''event.targetInfo.info''
  -- -- r4ae3Loh78v(振动仪)、k8dNIRut1q3（雷达）、xjWO7NdIOYs（天朗雷达）
  and product_key in(''r4ae3Loh78v'',''k8dNIRut1q3'',''xjWO7NdIOYs'');
...
select t1.tid,
       ...
       if(
                   coalesce(t2.objectLabel, t2.targetType) is null or coalesce(t2.objectLabel, t2.targetType) = '''' or
                   coalesce(t2.objectLabel, t2.targetType) = ''未知目标'',
                   ''未知'',
                   coalesce(t2.objectLabel, t2.targetType)
           )                                                               as object_label1,
...
select t1.*,
       if(t1.product_key = ''xjWO7NdIOYs'', t2.target_name, t1.object_label1) as object_label
...
from tmp_source_kafka_02
     -- QptZJHOd1KD :执法仪，zyrFih3kept(无人机),00000000002（机库子设备-无人机）
where
  -- product_key in(''QptZJHOd1KD'',''zyrFih3kept'',''00000000002'')
    `method` in (''properties.state'', ''event.property.post'') -- 所有设备轨迹数据全部入库
...
from (select type,
      ...
      where coalesce(`method`, message.`method`) in (''properties.state'', ''event.property.post'')
        and abs(coalesce(`timestamp`, message.`timestamp`) / 1000 - UNIX_TIMESTAMP()) <= 864000 -- 小于10天的数据
     ) as t1
         left join iot_device FOR SYSTEM_TIME AS OF t1.proctime as t2
                   on t1.device_id = t2.device_id
...
select device_id,
       ...
       ''                                                                                   PICTURE''  as type, from_unixtime(acquire_timestamp / 1000, ''yyyy-MM-dd HH:mm:ss'') as start_time,
...
where acquire_timestamp is not null
  and (
    (`method` = '' platform.capture.post ''
  and picture_url is not null) -- 截图
   or (`method` = '' event.mediaFileUpload.info ''
  and photo_url is not null
  and is_capture is null
  and SPLIT_INDEX(photo_url
    , ''.''
    , 1) <> '' MP4 '')         -- 拍照数据过滤截图的，如果是截图（is_capture=1）
    );
