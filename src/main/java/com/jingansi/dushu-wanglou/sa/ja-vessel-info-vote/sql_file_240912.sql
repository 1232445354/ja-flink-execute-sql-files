...
drop table if exists dim_vessel_ais_type_rel;
...
-- 船舶属性表（Source：doris）
drop table if exists dws_vessel_et_info_rt_source;
...
create table dim_vessel_c_name_info (
      ...
      'username' = 'root',
      'password' = 'Jingansi@110',
      'table-name' = 'dim_vessel_c_name_info',
      'driver' = 'com.mysql.cj.jdbc.Driver',
      'lookup.cache.max-rows' = '10000',
      'lookup.cache.ttl' = '86400s',
      'lookup.max-retries' = '10'
...
select
    ...
    t1.ais_type_name,
    cast(t1.class_code as int)         as vessel_class_code,
    t1.class_name                      as vessel_class_name,
    cast(t1.type_code as int)          as vessel_type_code,
    t1.type_name                       as vessel_type_name,
    t5.owner                        as owner,
    ...
    case
        when t1.country_code in('IN','US','JP','AU','TW') and class_code = '5' then 'ENEMY' -- 敌 - 美印日澳 军事
        when t1.country_code in ('CN','MO','HK') and class_code = '5' then 'OUR_SIDE'               -- 我 中国 军事
        when t1.country_code in ('CN','MO','HK') and  class_code <> '5' then 'FRIENDLY_SIDE'      -- 友 中国 非军事
        else 'NEUTRALITY'
        end as friend_foe,
