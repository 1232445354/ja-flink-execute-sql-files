...
create table iot_device_message_kafka_01 (
    ...
    -- 手动拍照截图数据
                                             `data`  row(
                                                 taskStatus string,
                                                 taskWaylineId int,
                                                 taskCurrentWaylineIndex int,
                                                 videoUrl            string, -- 媒体上报的录像url
...
create table dwd_device_track_rt (
                                     ...
                                     task_current_wayline_index bigint             comment '任务当前执行航点序号',
                                     task_wayline_id           bigint              comment '航线任务id'
)WITH (
     'connector' = 'doris',
     'fenodes' = 'doris-fe-service.bigdata-doris.svc.cluster.local:9999',   -- k8s部署
-- 'fenodes' = '172.21.30.202:30030',
...
select
    ...
    message.`data`.taskStatus  as task_status,
...
select
    ...
    if(
                coalesce(t2.objectLabel,t2.targetType) is null or coalesce(t2.objectLabel,t2.targetType) = '' or coalesce(t2.objectLabel,t2.targetType) = '未知目标',
                '未知',
...
from tmp_source_kafka_02
where acquire_timestamp is not null
  and `method` = 'event.videoMediaFileUpload.info' and video_url is not null;
