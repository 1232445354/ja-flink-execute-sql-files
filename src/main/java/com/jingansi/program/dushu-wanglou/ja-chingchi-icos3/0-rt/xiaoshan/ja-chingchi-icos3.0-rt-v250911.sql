...
-- version:ja-chingchi-icos3.0-rt-v250911 拍照数据视频数据重新入库
--********************************************************************--
...
(
    event      string,
    `time`     bigint,
    `data`     row< fileType string,
    filePath   string,
    ext        row < longitude double,
    latitude   double,
    height     double,
    createTime string > >
) WITH (
...
select t1.device_id,
       case
           when t1.`method` = ''platform.capture.post'' then ''SCREENSHOT''
           when t1.`method` = ''event.mediaFileUpload.info'' then ''PHOTOGRAPH''
           end                                                                                                    as source_id,
       ...
       if(`method` = ''platform.capture.post'', concat('' / '', picture_url),
          coalesce(photo_url, video_url))                                                                         as url,
       t1.longitude,
       t1.latitude,
       t1.width,
       t1.height,
...
from (select *
      ...
      where `method` = ''event.densityMap.info'') as t1
...
select t1.*,
       case
           when t1.product_key = ''xjWO7NdIOYs'' then t2.target_name
           when t1.product_key <> ''xjWO7NdIOYs'' and object_label1 is not null and object_label1 <> '''' and
       object_label1 <> '' 未知目标 '' then object_label1
      else ''未知''
end as object_label
...
select t1.device_id,
       ...
       t1.`type`,
       t1.operator,
       if(t1.type = ''properties'', JSON_VALUE(properties, ''$.healthInfo[0]''),
          JSON_VALUE(properties, ''$.message''))                                         as health_info
from (select type,
...
select ''                                                                  FILE_UPLOAD_CALLBACK'' as event, UNIX_TIMESTAMP(cast(start_time as varchar), ''yyyy-MM-dd HH:mm:ss'') * 1000 as `time`,
       get_row_udf(type, url, longitude, latitude, height, create_time) as data1
