...
select
    ...
    t1.latitude,
    t1.width,
    cast(t1.uav_height as int) as height,
    t1.bid,
    t1.tid,
    ...
    concat('{',
           trim(',' FROM concat(
                   ...
                   if(uav_height is null,'',concat('"height":"',cast(uav_height as string),'",')),
...
-- 拍照、视频 fh_sync 开头 反关联回来 写入open kafka
insert into file_upload_callback_sink_kafka
select
    'FILE_UPLOAD_CALLBACK' as event,
    UNIX_TIMESTAMP(cast(start_time as varchar),'yyyy-MM-dd HH:mm:ss') * 1000 as `time`,
...
-- 拍照、视频 fh_sync 开头 反关联回来后入库- mysql
insert into device_media_datasource
select
    device_id,
    source_id,
