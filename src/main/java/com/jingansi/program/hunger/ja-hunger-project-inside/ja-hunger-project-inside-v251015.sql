...
-- description: 饿了么数据存储，测试环境测试模型的
-- version：ja-hunger-project-inside-v251015
--********************************************************************--
set 'pipeline.name' = 'ja-hunger-project-inside';
...
-- SET 'state.checkpoints.dir' = 's3://ja-flink/flink-checkpoints/ja-hunger-project-inside' ;



-- kafka来源（Source：kafka）
create table test_infer_result (
    ...
    >,
      ...
      'properties.group.id' = 'test_infer_result3-group',
      -- 'scan.startup.mode' = 'latest-offset',
      'scan.startup.mode' = 'timestamp',
      'scan.startup.timestamp-millis' = '1760063114000',
      'format' = 'json',
...
create table dwd_hunger_all_rt_inside (
     ...
     'table.identifier' = 'hunger.dwd_hunger_all_rt_inside',
     'username' = 'root',
...
-- 商家数据入库 （Sink：doris）
create table dwd_hunger_all_rt_inside_simp (
                                               batch_id                bigint      	comment '批处理ID',
                                               ntp_timestamp_format    timestamp       comment '时间戳格式化',
                                               ntp_timestamp           bigint      	comment '时间戳',
                                               is_cover                boolean,
     ...
     'connector' = 'doris',
     'fenodes' = 'doris-fe-service.bigdata-doris.svc.cluster.local:9999',
     'table.identifier' = 'hunger.dwd_hunger_all_rt_inside_simp',
     'username' = 'root',
     'password' = 'Jingansi@110',
     'doris.request.tablet.size'='1',
     'doris.request.read.timeout.ms'='30000',
...
create view test_infer_result1 as
select * from test_infer_result
where `partition` = 1;
...
-- 商家数据入库
insert into dwd_hunger_all_rt_inside_simp
select
    batch_id,
    TO_TIMESTAMP_LTZ(ntp_timestamp,3) as ntp_timestamp_format, -- 时间戳格式化
    ntp_timestamp,
    is_cover,
...
from test_infer_result1;


-- 给定饿了吗的数据-全量数据写入doris
insert into dwd_hunger_all_rt_inside
select
    object_id,
