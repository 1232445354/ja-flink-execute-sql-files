...
create table test_infer_result (
    ...
    -- watermark for rowtime as rowtime - interval '5' second
      ...
      -- -- 'scan.startup.timestamp-millis' = '1739499521000',
      -- 'format' = 'json',
...
-- 商家数据入库 （Sink：doris）
create table dwd_hunger_all_rt_test_simp (
                                             batch_id                bigint      	comment '批处理ID',
                                             ntp_timestamp_format    timestamp       comment '时间戳格式化',
                                             ntp_timestamp           bigint      	comment '时间戳',
                                             is_cover                boolean,
     ...
     'connector' = 'doris',
     'fenodes' = 'doris-fe-service.bigdata-doris.svc.cluster.local:9999',
     'table.identifier' = 'hunger.dwd_hunger_all_rt_test_simp',
     'username' = 'root',
     'password' = 'Jingansi@110',
     'doris.request.tablet.size'='1',
     'doris.request.read.timeout.ms'='30000',
...
-- 商家数据入库
insert into dwd_hunger_all_rt_test_simp
select
    batch_id,
