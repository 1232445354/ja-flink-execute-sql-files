...
set
''pipeline.name'' = ''ja-uav-single-table'';
...
300000'';
SET
''state.checkpoints.dir'' = ''s3://flink/flink-checkpoints/ja-uav-single-table'';
...
(
    ...
    uav_id                    string comment '' uav无人机id,
    sn号 '',
    ...
    recvtype                  string comment '' 无人机数据类型，示例：2.4 G '',
...
select device_id,                                             -- AOA的设备ID
       ...
       confidence
from (select *,
             if(aoa_uav_id = '''' or aoa_uav_id is null or trim(aoa_uav_id) = '''' or trim(aoa_uav_id) is null,
                cast(null as varchar), trim(aoa_uav_id)) as aoa_uav_id_trim
      from temp01
  ...
  and control_station_latitude <> 0) as tt1,
     lateral table(PassThroughUdtf(get_random_id())) as b(uav_random_id);
