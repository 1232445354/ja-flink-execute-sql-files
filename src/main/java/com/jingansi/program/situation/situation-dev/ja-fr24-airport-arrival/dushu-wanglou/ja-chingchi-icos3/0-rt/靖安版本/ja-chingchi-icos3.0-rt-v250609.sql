...
-- version:ja-chingchi-icos3.0-rt-v250609 天朗雷达新增入库字段
-- 和上个版本的拍照经纬度没有合并代码呢
--********************************************************************--
...
create table dwd_radar_target_all_rt(
                                        ...
                                        time1                      string     ,
                                        upload_mode                string     , -- 模式
                                        target_state               bigint     , -- 目标状态
                                        tid                        string     , -- 当前请求的事务唯一ID
                                        bid                        string     , -- 长连接整个业务的ID
                                        `method`                   string     , -- 服务&事件标识
...
select
    ...
    if(
                coalesce(t2.objectLabel,cast(t2.targetType as varchar)) is null
                or coalesce(t2.objectLabel,cast(t2.targetType as varchar)) = ''
                or coalesce(t2.objectLabel,cast(t2.targetType as varchar)) = '未知目标',
                '未知',
                coalesce(t2.objectLabel,cast(t2.targetType as varchar))
        ) as object_label1,
...
from tmp_source_kafka_02
where `method` in('properties.state','event.property.post')  -- 所有设备轨迹数据全部入库
