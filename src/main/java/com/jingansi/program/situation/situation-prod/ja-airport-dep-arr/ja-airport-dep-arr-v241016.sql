...
-- version: ja-airport-dep-arr-v240116
--********************************************************************--
...
create table fr24_airport_departure_kafka(
                                             ...
                                             status_live               string, -- 是否活动，飞行中 False
                                             ...
                                             airport_from_country      string, -- 出发机场国家 英文名称
                                             ...
                                             airport_to_country        string, -- 到达机场国家 英文名称
...
create table fr24_airport_on_ground_kafka(
                                             ...
                                             hours_diff                string, -- 飞机航班最后更新时间 单位：小时
                                             time_diff                 string, -- 飞机航班最后更新时间 单位：秒
...
create table dws_bhv_airport_flight_time (
                                             id                        string, -- 机场id
                                             src_code                  string, -- 起飞降落类型，flight_departure-起飞，flight_arrival-到达，on_ground 地面
                                             acquire_time              string, -- 采集时间string
                                             update_time               string
) with (
      'connector' = 'doris',
...
create table dwd_bhv_airport_onground_rt (
                                             ...
                                             on_ground_update     	    string, -- 在地面时间 和时间戳时一样的
                                             hours_diff                double, -- 飞机航班最后更新时间 单位：小时
                                             time_diff 				double, -- 飞机航班最后更新时间 单位：秒
...
select * from fr24_airport_departure_kafka
union all
...
select
    flight_airport,                                        -- 飞机机场 iata或者icao,大师写入的 小写的代码
    flight_type         as src_code,                       -- 飞机降落标志
    acquire_time,                                          -- 采集时间，年月日时分秒格式的
    cast(acquire_timestamp as bigint) as acquire_timestamp,

    if(flight_number = '',cast(null as varchar),flight_number)       as flight_number,  -- 航班号
    ...
    if(aircraft_model = '',cast(null as varchar),aircraft_model)     as aircraft_type,  -- 来源的-飞机机型
    aircraft_desc,   -- 飞机类型描述
    aircraft_date,   -- 飞机启用时间
    if(airline_name = '',cast(null as varchar),airline_name) as airline_name, -- 航空公司英文名称
    if(airline_iata = '',cast(null as varchar),airline_iata) as airline_iata, -- 航空公司iata
    if(airline_icao = '',cast(null as varchar),airline_icao) as airline_icao, -- 航空公司icao

    airport_from_name,     -- 英文名称
    airport_from_iata,
    airport_from_icao,
    airport_from_country,    -- 国家名称 英文
    airport_from_city,
    ...
    if(std_bj_time = '',cast(null as varchar),std_bj_time) as std_bj_time, -- 计划出发时间(北京时间)
    if(sta_bj_time = '',cast(null as varchar),sta_bj_time) as sta_bj_time, -- 计划到达时间(北京时间)
    if(atd_bj_time = '',cast(null as varchar),atd_bj_time) as atd_bj_time, -- 实际起飞时间(北京时间)
    if(ata_bj_time = '',cast(null as varchar),ata_bj_time) as ata_bj_time, -- 实际降落时间(北京时间)
    if(eta_bj_time = '',cast(null as varchar),eta_bj_time) as eta_bj_time, -- 预计降落时间(北京时间)
...
select
    coalesce(if(src_code = 'flight_departure',t2.id,t3.id),t1.flight_airport) as id,
    src_code,       -- 起飞降落标志
    acquire_time,
    coalesce(hex,f24_id)          as flight_id,
...
from tmp_02 as t1
         left join dws_et_airport_info FOR SYSTEM_TIME AS OF t1.proctime as t2
                   on if(t1.airport_from_icao is not null,t1.airport_from_icao,t1.airport_from_iata) = t2.id    -- 起飞

         left join dws_et_airport_info FOR SYSTEM_TIME AS OF t1.proctime as t3
                   on if(t1.airport_to_icao is not null,t1.airport_to_icao,t1.airport_to_iata) = t3.id     -- 到达

         left join dws_et_aircraft_info FOR SYSTEM_TIME AS OF t1.proctime as t4      -- 飞机实体
                   on coalesce(t1.hex,t1.f24_id) = t4.flight_id;
...
from fr24_airport_on_ground_kafka
     -- flight_type不是飞机类型，而是航班的类型
where flight_type is not null;
...
select
    coalesce(t2.id,t1.flight_airport) as id,
    acquire_time,
    coalesce(hex,f24_id) as flight_id,
    ...
    hex,
    aircraft_reg,
    coalesce(t1.aircraft_type,t4.flight_type) as aircraft_type, -- 机型
    aircraft_desc, -- 类型描述
    airline_name,  --航空公司名称 英文
    airline_iata,  -- 航空公司iata
    airline_icao,  -- 航空公司icao
    t4.country_code,
    t4.country_name,
    ...
    coalesce(t1.airport_name,t2.e_name) as airport_e_name,
...
from tmp_ground_01 as t1
         left join dws_et_airport_info FOR SYSTEM_TIME AS OF t1.proctime as t2
                   on if(t1.airport_icao is not null,t1.airport_icao,t1.airport_iata) = t2.id    -- 起飞
