...
--version: ja-vessel-info-vote-v250716 新增myshiptracking 属性投票
--********************************************************************--
...
--   as t1
--   left join dws_vessel_et_info_rt_source FOR SYSTEM_TIME AS OF t1.proctime as t2
...
select
    cast(t1.newVesselId as bigint) as new_vessel_id,
    t1.mmsi,
...
select
    tmp_01.new_vessel_id,
    tmp_01.mmsi,
    tmp_01.proctime,
    b.vote_result
from tmp_01 ,lateral table(map_repeated_call_udtf(atr_vote_udf(marinetrafficInfo,vessel1,vt_map,mmsi))) as b(vote_result);
...
select
    ...
    vote_result['owner'] as owner,
    vote_result['gt'] as gt,
    vote_result['dwt'] as dwt
from tmp_02;
...
select
    new_vessel_id                                            as vessel_id,
    coalesce(t1.e_name,t2.vessel_name)                       as vessel_name,
    t2.vessel_c_name                                         as vessel_c_name,
    coalesce(cast(t1.mmsi as bigint),t2.mmsi)                as mmsi,
    coalesce(cast(t1.imo as bigint),t2.imo)                  as imo,
    coalesce(t1.callsign,t2.callsign)                        as callsign,
    coalesce(cast(t1.length as double),t2.length)            as length,
    coalesce(cast(t1.width as double),t2.width)              as width,
    t2.height                                                as height,
    coalesce(cast(t1.gt as double),t2.gross_tonnage)         as gross_tonnage,
    coalesce(cast(t1.dwt as double),t2.deadweight)           as deadweight,
    coalesce(t1.year_built,t2.build_year)                    as build_year,
    coalesce(t1.service_status,t2.service_status_code)       as service_status_code,
    coalesce(t4.c_name,t2.service_status_name)               as service_status_name,
    coalesce(t1.home_port,t2.registry_port)                  as registry_port,
    coalesce(t1.country_code,t2.country_code)                as country_code,
    coalesce(t3.c_name,t2.country_name)                      as country_name,
    coalesce(cast(t1.ais_type_code as int),t2.ais_type_code) as ais_type_code,
    coalesce(t1.ais_type_name,t2.ais_type_name)              as ais_type_name,
    coalesce(t2.vessel_class_code,cast(t1.class_code as int))as vessel_class_code,
    coalesce(t2.vessel_class_name,t1.class_name)             as vessel_class_name,
    coalesce(t2.vessel_type_code,cast(t1.type_code as int))  as vessel_type_code,
    coalesce(t2.vessel_type_name,t1.type_name)               as vessel_type_name,
    coalesce(t1.owner,t2.owner)                              as owner,
    t2.check_cols

from tmp_03 as t1
         left join dws_vessel_et_info_rt_source FOR SYSTEM_TIME AS OF t1.proctime as t2     -- 实体表
                   on t1.new_vessel_id = t2.vessel_id

         left join dim_country_code_name_info FOR SYSTEM_TIME AS OF t1.proctime as t3        -- 国家名称
                   on t1.country_code = t3.country_code2

         left join dim_mtf_service_status_info FOR SYSTEM_TIME AS OF t1.proctime as t4       -- 服务状态
                   on t1.service_status = t4.e_name

where t2.check_cols <> '["all-checked"]'
   or check_cols is null;
...
select
    *,
    concat(
            ifnull(vessel_name,''),' ',
            ifnull(vessel_c_name,''),' ',
            ifnull(cast(mmsi as varchar),''),' ',
            ifnull(cast(imo as varchar),'')
        ) as search_content,
    case
        when country_code in('IN','US','JP','AU','TW') and vessel_class_code = 5 then '1' -- 敌 - 美印日澳 军事
        when country_code in ('CN','MO','HK') and vessel_class_code = 5 then '2'               -- 我 中国 军事
        when country_code in ('CN','MO','HK') and  vessel_class_code <> 5 then '3'      -- 友 中国 非军事
        else '4'
        end as friend_foe
