...
-- -- SET 'state.checkpoints.dir' = 's3://flink/flink-checkpoints1' ;
...
drop table if exists ais_mob_mysql;
create table ais_mob_mysql (
...
drop table if exists black_white_list_mysql;
create table black_white_list_mysql (
    id              bigint  comment '主键',
...
-- ais、radar融合的实时位置状态表（Sink：doris）
drop table if exists merge_target_ship_info_doris;
create table merge_target_ship_info_doris (
...
'table.identifier' = 'chingchi_icos.merge_target_ship_info',
'username' = 'root',
'password' = 'root',
...
'connector' = 'doris',
'fenodes' = '172.21.30.202:30030',
'table.identifier' = 'chingchi_icos.event_alarm_info',
'username' = 'root',
'password' = 'root',
'doris.request.tablet.size'='1',
'doris.request.read.timeout.ms'='30000',
...
drop table if exists site_mysql;
create table site_mysql (
    id                  bigint        comment '主键',
...
-- AIS状态表（Sink：doris）
drop table if exists ais_target_ship_info_doris;
create table ais_target_ship_info_doris (
...
'connector' = 'doris',
'fenodes' = '172.21.30.202:30030',
'table.identifier' = 'chingchi_icos.ais_target_ship_info',
'username' = 'root',
'password' = 'root',
'doris.request.tablet.size'='1',
'doris.request.read.timeout.ms'='30000',
...
-- ais船只全量数据表（Sink：doris）
drop table if exists ais_target_ship_all_info_doris;
create table ais_target_ship_all_info_doris (
    mmsi                string,
...
'connector' = 'doris',
'fenodes' = '172.21.30.202:30030',
'table.identifier' = 'chingchi_icos.ais_target_ship_all_info',
'username' = 'root',
'password' = 'root',
'doris.request.tablet.size'='1',
'doris.request.read.timeout.ms'='30000',
...
-- radar雷达全量数据信息表（Sink：doris）
drop table if exists radar_target_ship_all_info_doris;
create table radar_target_ship_all_info_doris (
    radar_id        string,
    target_id       string,
    receive_time    timestamp,
    distance        double,
...
'connector' = 'doris',
'fenodes' = '172.21.30.202:30030',
'table.identifier' = 'chingchi_icos.radar_target_ship_all_info',
'username' = 'root',
'password' = 'root',
'doris.request.tablet.size'='1',
'doris.request.read.timeout.ms'='30000',
...
--  ais、radar融合对应关系表（Sink：doris）
drop table if exists target_ship_relation_info_sink_doris;
create table target_ship_relation_info_sink_doris (
...
'connector' = 'doris',
'fenodes' = '172.21.30.202:30030',
'table.identifier' = 'chingchi_icos.target_ship_relation_info',
'username' = 'root',
'password' = 'root',
'doris.request.tablet.size'='1',
'doris.request.read.timeout.ms'='30000',
...
drop table if exists target_ship_relation_info_source_doris;
create table target_ship_relation_info_source_doris (
	target_id 		string     comment '雷达目标id',
    mmsi 			string	   comment '船只唯一标识',
    earliest_time   timestamp  comment '最早时间',
    latest_time     timestamp  comment '最晚时间',
    ...
    'connector' = 'jdbc',
    'url' = 'jdbc:mysql://doris-fe-service.bigdata-doris.svc.cluster.local:31030/chingchi_icos?useSSL=false&useUnicode=true&characterEncoding=UTF-8&characterSetResults=UTF-8&zeroDateTimeBehavior=CONVERT_TO_NULL&serverTimezone=UTC',
    'username' = 'root',
    'password' = 'root',
...
-- ais、radar融合全量数据表（Sink：doris）
drop table if exists merge_target_ship_all_info_doris;
create table merge_target_ship_all_info_doris (
  id 			string       comment '船只唯一标识，mmsi或是target_id',
...
'connector' = 'doris',
'fenodes' = '172.21.30.202:30030',
'table.identifier' = 'chingchi_icos.merge_target_ship_all_info',
'username' = 'root',
'password' = 'root',
'doris.request.tablet.size'='1',
'doris.request.read.timeout.ms'='30000',
...
-- 两个radar雷达的目标对应关系数据表(Sink:doris)
drop table if exists merge_radar_target_ship_relation_info_doris;
create table merge_radar_target_ship_relation_info_doris (
...
'connector' = 'doris',
'fenodes' = '172.21.30.202:30030',
'table.identifier' = 'chingchi_icos.merge_radar_target_ship_relation_info',
'username' = 'root',
'password' = 'root',
'doris.request.tablet.size'='1',
'doris.request.read.timeout.ms'='30000',
...
-- 雷达的状态数据信息表(Sink:doris)
drop table if exists radar_target_ship_info_doris;
create table radar_target_ship_info_doris (
...
'connector' = 'doris',
'fenodes' = '172.21.30.202:30030',
'table.identifier' = 'chingchi_icos.radar_target_ship_info',
'username' = 'root',
'password' = 'root',
'doris.request.tablet.size'='1',
'doris.request.read.timeout.ms'='30000',
...
-- 雷达数据进入数据的最小时间（延迟告警）(Sink:doris)
drop table if exists radar_target_ship_entry_info_sink_doris;
create table radar_target_ship_entry_info_sink_doris (
  target_id      string      comment '目标id',
...
'connector' = 'doris',
'fenodes' = '172.21.30.202:30030',
'table.identifier' = 'chingchi_icos.radar_target_ship_entry_info',
'username' = 'root',
'password' = 'root',
'doris.request.tablet.size'='1',
'doris.request.read.timeout.ms'='30000',
...
drop table if exists radar_target_ship_entry_info_source_doris;
create table radar_target_ship_entry_info_source_doris (
    target_id      string   	 comment '目标id',
    earliest_time  timestamp     comment '雷达数据进入的最小时间',
  primary key (target_id) NOT ENFORCED
) with (
...
-- 雷达融合的状态数据信息表(Source-Sink:doris)
drop table if exists merge_radar_target_ship_info_doris;
create table merge_radar_target_ship_info_doris (
  target_id  	 string 	 comment '目标id',
  radar_id       string   	 comment '雷达id',
  receive_time   timestamp   comment '接收时间',
  distance       double      comment '距离',
...
'connector' = 'doris',
'fenodes' = '172.21.30.202:30030',
'table.identifier' = 'chingchi_icos.merge_radar_target_ship_info',
'username' = 'root',
'password' = 'root',
'doris.request.tablet.size'='1',
'doris.request.read.timeout.ms'='30000',
...
-- ais和雷达的对应关系数据入库，主键为mmsi（Sink：doris)
drop table if exists target_ship_relation_ais_info_sink_doris;
create table target_ship_relation_ais_info_sink_doris (
    mmsi 			string	   comment '船只唯一标识',
	target_id 		string     comment '雷达目标id',
    earliest_time   timestamp  comment '最早时间',
    latest_time     timestamp  comment '最晚时间',
...
'connector' = 'doris',
'fenodes' = '172.21.30.202:30030',
'table.identifier' = 'chingchi_icos.target_ship_relation_ais_info',
'username' = 'root',
'password' = 'root',
'doris.request.tablet.size'='1',
'doris.request.read.timeout.ms'='30000',
...
drop table if exists target_ship_relation_ais_info_source_doris;
create table target_ship_relation_ais_info_source_doris (
    mmsi 			string	   comment '船只唯一标识',
	target_id 		string     comment '雷达目标id',
    earliest_time   timestamp  comment '最早时间',
    latest_time     timestamp  comment '最晚时间',
...
select
  ...
  -- from_unixtime(t1.receiveTime/1000,'yyyy-MM-dd HH:mm:ss') as receive_time, -- 接收时间timestamp类型
  -- to_timestamp(from_unixtime(t1.receiveTime/1000,'yyyy-MM-dd HH:mm:ss'),'yyyy-MM-dd HH:mm:ss') as receive_time, -- 接收时间timestamp类型
  to_timestamp(from_unixtime(t1.receiveTime,'yyyy-MM-dd HH:mm:ss'),'yyyy-MM-dd HH:mm:ss') as receive_time, -- 接收时间timestamp类型
  ...
  distance_udf(
              ...
              t2.latitude,
              t2.longitude) * 1000 as site_distance
  from ja_target_radar_kafka as t1
left join site_mysql
...
create view tmp_ja_target_ais_02 as
  ...
  from (
      ...
      where mmsi is not null
        ...
        and char_length(cast(receiveTime as string)) <= 10
  ) as t1
  left join site_mysql
  FOR SYSTEM_TIME AS OF t1.proctime as t2
  on 'DRILLING_PLATFORM' = t2.type
...
select
  ...
  from (
      select
        *,
        row_number() over(partition by target_id,receive_time order by ais_radar_target_distance asc,date_diff_flag asc) as rk
      ...
      from (
          select
              ...
              abs(timestampdiff(second,t1.receive_time,t2.receiveTime)) as date_diff_flag,
...
from tmp_ja_target_radar_04 as t1
left join target_ship_relation_info_source_doris
FOR SYSTEM_TIME AS OF t1.proctime as t2
...
select
 ...
 left join black_white_list_mysql                     -- 黑白名单信息表
 FOR SYSTEM_TIME AS OF t1.proctime as t2
   on t1.mmsi = t2.mmsi
 left join radar_target_ship_entry_info_source_doris  -- 雷达数据最小时间表
FOR SYSTEM_TIME AS OF t1.proctime as t3
...
select
  ...
  from (
    select
      *
      from tmp_ja_target_ais_02
      where person_identification = '991'
        and messageType = '14'
  ) as t1
left join ais_mob_mysql                    -- 人员基本信息
FOR SYSTEM_TIME AS OF t1.proctime as t2
...
left join target_ship_relation_ais_info_source_doris   -- 雷达ais对应关系表
FOR SYSTEM_TIME AS OF t1.proctime as t2
on t1.mmsi = t2.mmsi
...
-- 雷达融合的状态数据信息表
insert into merge_radar_target_ship_info_doris
select
  target_id,
  radar_id,
...
-- 雷达数据进入数据的最小时间（延迟告警）
insert into radar_target_ship_entry_info_sink_doris
select
  target_id,
  min(receive_time) as earliest_time,
  max(receive_time) as latest_time,
...
insert into radar_target_ship_info_doris
select
  radar_id,
  target_id,
  receive_time,
...
-- ais船只状态信息数据入库doris
insert into ais_target_ship_info_doris
select
    mmsi               ,
    ais_id             ,
    index_name         ,
...
-- ais船只全量表数据入库doris
insert into ais_target_ship_all_info_doris
select
    mmsi                              ,
    receiveTime                       as receive_time    ,
    imo                               ,
...
-- radar雷达的全量数据入库doris
insert into radar_target_ship_all_info_doris
select
  radar_id        ,
  target_id       ,
  receive_time    ,
...
-- ais和radar雷达对应关系数据入库doris
insert into target_ship_relation_info_sink_doris
select
   	target_id 		               , -- 雷达目标id
    mmsi 			               , -- 船只唯一标识
...
-- ais和radar雷达对应关系数据入库doris,主键mmsi
insert into target_ship_relation_ais_info_sink_doris
select
    mmsi 			               , -- 船只唯一标识
   	target_id 		               , -- 雷达目标id
    earliest_time as earliest_time , -- 最早融合时间
  ...
  -- 两个雷达融合结果对应关系数据入库doris
 insert into merge_radar_target_ship_relation_info_doris
  select
...
-- insert into merge_target_ship_info_doris
-- select
--     mmsi                 as id   ,
--     'ais'                as type,
--     receiveTime          as receive_time,
...
-- ais和雷达融合数据的实时位置数据入库doris（融合状态表）
insert into merge_target_ship_info_doris
  select
...
-- -- ais的全数据入库doris（融合轨迹表）
-- insert into merge_target_ship_all_info_doris
-- select
--   mmsi                              , -- 船只mmsi
--   'ais' as type                     , -- 类型
