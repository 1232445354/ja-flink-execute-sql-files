...
-- description: 旌旗3.0设备接入日志、设备检测数据状态，老版本的雷达数据data里面直接对象的，临时改的
...
create table iot_device_message_kafka (
  ...
  -- 'scan.startup.timestamp-millis' = '1711557376000',
  'format' = 'json',
...
-- 设备状态上报数据入库（无人机子设备、无人机、执法仪）（Sink：mysl）-- 纵表
...
select
  ...
  target_altitude,
  cast(target_longitude as varchar) as target_longitude,
  cast(target_latitude as varchar) as target_latitude,
  target_pitch,
  message_tid as tid,
  message_bid as bid,
...
-- 设备状态上报数据入库（无人机子设备、无人机、执法仪）（Sink：mysl） -- 数据存储过大，所以存储在了doris数据库中
