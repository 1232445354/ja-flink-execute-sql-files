...
create table dwd_device_attr_info (
                                      ...
                                      properties                string          comment '设备属性',
                                      tid                       string          comment '当前请求的事务唯一ID',
                                      bid                       string          comment '长连接整个业务的ID',
                                      `method`                  string          comment '服务&事件标识',
...
where t1.device_id is not null
  -- and t1.acquire_timestamp is not null  -- 2024-01-01 16:00:00
...
select
    ...
    target_id,
    concat('[{',
           concat('"deviceName":"',device_name_join,'",'),
...
select
    t1.device_id,
    case
        when t1.product_key = 'Y95SjAkrmRG' then '1'
        when t1.product_key = 'uvFrSFW2zMs' then '2'
        when t1.product_key = 'mVpLCOnTPLz' then '3'
        when t1.product_key = 'k8dNIRut1q3' then '4'
        when t1.product_key = 'raYeBHvRKYP' then '5'
        when t1.product_key = 'r4ae3Loh78v' then '7'
        end as device_type,
    if(t1.product_key = 'Y95SjAkrmRG',t1.device_id,t2.parent_id) as parent_id,
    TO_TIMESTAMP_LTZ(acquire_timestamp,3) as acquire_timestamp_format,
    acquire_timestamp,
    properties,
    tid,
...
from (
         ...
         where message.productKey in('Y95SjAkrmRG','uvFrSFW2zMs','mVpLCOnTPLz','k8dNIRut1q3','raYeBHvRKYP','r4ae3Loh78v')  -- Y95SjAkrmRG（望楼）、uvFrSFW2zMs(可见光)、mVpLCOnTPLz(红外)、k8dNIRut1q3(雷达)、raYeBHvRKYP(北斗)、r4ae3Loh78v(振动仪)
           and message.`method`= 'properties.state'
     ) as t1
         left join iot_device FOR SYSTEM_TIME AS OF t1.proctime as t2
                   on t1.device_id = t2.device_id
where t1.device_id is not null
  -- and t1.acquire_timestamp is not null
  and t1.acquire_timestamp > 1704096000000;
