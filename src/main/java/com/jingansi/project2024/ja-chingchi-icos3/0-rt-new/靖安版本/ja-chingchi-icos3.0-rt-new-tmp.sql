...
create table iot_device (
     ...
     'url' = 'jdbc:mysql://172.21.30.201:31306/dushu?useSSL=false&characterEncoding=UTF-8&serverTimezone=GMT%2B8',
     'username' = 'root',
     'password' = 'jingansi110',
...
-- 各种设备属性存储 - 望楼1,可见光2,红外3,雷达4,北斗5,电池6,震动仪7
drop view if exists tmp_attr_01;
create view tmp_attr_01 as
select
    device_id,
    case
        when product_key = 'Y95SjAkrmRG' then '1'
        when product_key = 'uvFrSFW2zMs' then '2'
        when product_key = 'mVpLCOnTPLz' then '3'
        when product_key = 'k8dNIRut1q3' then '4'
        when product_key = 'raYeBHvRKYP' then '5'
        when product_key = 'r4ae3Loh78v' then '7'
        end as device_type,
    if(product_key = 'Y95SjAkrmRG',device_id,parent_id) as parent_id,
    TO_TIMESTAMP_LTZ(acquire_timestamp,3) as acquire_timestamp_format,
    acquire_timestamp,
    longitude,
    latitude,
    ...
    type
from tmp_source_kafka_02
where product_key in('Y95SjAkrmRG','uvFrSFW2zMs','mVpLCOnTPLz','k8dNIRut1q3','raYeBHvRKYP','r4ae3Loh78v') -- Y95SjAkrmRG（望楼）、uvFrSFW2zMs(可见光)、mVpLCOnTPLz(红外)、k8dNIRut1q3(雷达)、raYeBHvRKYP(北斗)、r4ae3Loh78v(振动仪)
  and `method` = 'properties.state';
