...
"2025-04-03 00:00:00 2025-11-01 00:00:00"
)
...
col="dwd_weather_indices_all weather_time 3600"
...
for time_range in "${time_ranges[@]}"; do
  ...
  while [ "$current_timestamp1" -le "$end_timestamp" ]; do
    ...
    for ((current = minLong; current <= maxLong - step; current += step)); do
        ...
        echo -e "${current_day1} + ${current_day2} 范围: [$current_min, $current_max]"
        while true; do
          sh ${DIR}/sql_file.sh "$current_day1" "$current_day2" "$table_name" "$time_column" "$catalog_info" "$current_min" "$current_max"
          if [ $? -eq 0 ]; then
            echo -en "执行成功\n"
            break
          else
