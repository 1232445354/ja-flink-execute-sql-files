...
if curl -s -I -L  -x $proxy_ip:$proxy_port http://www.baidu.com | grep HTTP; then
    ...
    echo "Proxy $proxy_nam[$proxy_ip:$proxy_port] is not accessible."
fi
...
cmd="nohup /usr/local/bin/pproxy -l http+socks4+socks5://${proxy_ip}:${proxy_port} -r ssr://chacha20:${service_key}@${service_ip}:${service_port} &"
