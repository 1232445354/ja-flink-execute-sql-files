...
-- version: ja-uav-detection-target-rt-v250630 关联设备表-取值设备类型
--********************************************************************--
...
from (
         ...
         where `method` = 'event.targetInfo.info'
           and product_key in ('00000000002','zyrFih3kepx','zyrFih3kept')
     ) as t1
         left join device FOR SYSTEM_TIME AS OF t1.proctime as t2   -- 设备表 关联设备类型
...
select
    ...
    coalesce(t1.source_type,t2.sourceType)  as source_type,
