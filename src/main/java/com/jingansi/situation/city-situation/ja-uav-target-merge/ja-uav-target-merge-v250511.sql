...
-- description: rid、aoa、雷达数据融合
-- version: ja-uav-target-merge-v250511
--********************************************************************--

set
''pipeline.name'' = ''ja-uav-target-merge'';
...
120000'';
SET
''state.checkpoints.dir'' = ''s3://flink/flink-checkpoints/ja-uav-target-merge'';
...
-- 融合获取id
create function get_id as ''com.jingan.udf.merge.TargetTimeAndSpaceMerge'';

-- SET ''pipeline.global-job-parameters'' = ''
--   mysqlUrl:"jdbc:mysql://172.21.30.244:9030/sa?useSSL=false&characterEncoding=UTF-8&serverTimezone=GMT%2B8",
...
(
    ...
    basic_info                      row< basic_uaid string,       -- UAVID 无人机序列号
    basic_idtype                    bigint,
    basic_uatype                    bigint                        -- UA 类型     1：设备序列号 2：由 CAA 提供的 UAS 实名登记号 3：UTM 任务 ID （3-0 位，UA 类型）
        >,
    location_info                   row< location_speed_v double, -- 垂直速度
    ...
    location_direc                  double                        -- 保留字
        >,
    system_info                     row< alit double,             -- 控制站高度
...
(
    ...
    recvtype                  string comment '' 无人机数据类型,
    示例:2.4 G '',
...
select productKey                                                                         as product_key,
       ...
       coalesce(message.`data`.system_info.sys_lon,
                message.`data`.controllerLongitude)                                       as control_station_longitude, -- 控制站经度
       coalesce(message.`data`.system_info.sys_lat,
                message.`data`.controllerLatitude)                                        as control_station_latitude,  -- 控制站纬度
...
select t1.acquire_timestamp,
       t1.product_key,
       t1.device_id,
       t1.device_name,
       t1.acquire_time,
       t1.`method`,
       ''                       RADAR''               as src_code, t2.targetId as src_pk,
...
from (select *
      from temp02
      where `method` = ''event.targetInfo.info'') as t1
...
-- get_id(id,lng,lat,`time`*1000,type) as res_str,
