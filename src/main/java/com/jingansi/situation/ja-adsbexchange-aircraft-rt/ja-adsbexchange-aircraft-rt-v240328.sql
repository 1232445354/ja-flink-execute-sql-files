...
select
    ...
    case
        when countryCode in ('IN','US','JP','AU','TW') and isMilitary=1 then 'ENEMY' -- 敌(印度、美国、日本、澳大利亚、台湾 军事)
        when countryCode in ('CN','HK','MO') and isMilitary=1  then 'OUR_SIDE'       -- 我(中国	军事)
        when countryCode in ('CN','HK','MO') and isMilitary=0  then 'FRIENDLY_SIDE'  -- 友(中国	非军事)
        else 'NEUTRALITY' end friendFoe -- 敌我类型
