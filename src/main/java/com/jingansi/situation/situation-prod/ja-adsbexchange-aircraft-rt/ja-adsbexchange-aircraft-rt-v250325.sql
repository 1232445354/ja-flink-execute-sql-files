...
-- description: adsbexchange飞机轨迹(融合、单独)
-- version: ja-adsbexchange-aircraft-rt-v250325
--********************************************************************--
set 'pipeline.name' = 'ja-adsbexchange-aircraft-rt';


set 'parallelism.default' = '60';
...
create table adsb_exchange_aircraft_list_kafka(
      ...
      -- 'scan.startup.timestamp-millis' = '1745056200000',
      'format' = 'json',
...
select
    upper(hex)                                                     as id,               -- 飞机标识字段  1. 24位 icao编码 2. 来源站的标识如 a. radarbox flight_trace_id  b. adsbexchange ～开头的编码
    acquire_timestamp_format                                       as acquireTime,      -- 采集时间
    2                                                              as srcCode,          -- 来源网站标识 1. radarbox 2. adsbexchange
    upper(hex)                                                     as icaoCode,         -- 24位 icao编码
    coalesce(if(r='',cast(null as string),r),c.registration)       as registration,     -- 注册号
    flight                                                         as flightNo,         -- 航班号
    cast(null as string)                                           as callsign,         -- 呼号
    coalesce(if(t='',cast(null as string),t),c.flight_type)        as flightType,       -- 飞机型号
    db_flags % 2                                                   as isMilitary,       -- 是否军用飞机 0 非军用 1 军用
  if(left(hex,1)='~','non_icao','hex')                           as pkType,           -- flight_id 主键的类型 hex： icao hex 24位编码 trace_id：radarbox 的追踪id non_icao: adsbexchange 不是真正的 hex
  hex                                                            as srcPk,            -- 源网站主键
  b.category_code                                                as flightCategory,  -- 飞机类型
  b.category_c_name                                              as flightCategoryName, -- 飞机类型名称
  lon                                                            as lng,              -- 经度
  lat, -- 纬度
  gs                                                             as speed,            -- 飞行当时的速度（单位：节）
  cast(gs as double)*1.852                                       as speedKm,         -- 速度单位 km/h
  alt_baro                                                       as altitudeBaro,    -- 气压高度 海拔 航班当前高度，单位为（ft）
  cast(alt_baro as double)*0.3048                                as altitudeBaroM,  -- 气压高度 海拔 单位米
  alt_geom                                                       as altitudeGeom,    -- 海拔高度 海拔 航班当前高度，单位为（ft）
  cast(alt_geom as double)*0.3048                                as altitudeGeomM,  -- 海拔高度 海拔 单位米
  track                                                          as heading,          -- 方向  正北为0
  squawk                                                         as squawkCode,      -- 当前应答机代码

  case when airground = 1 then 1  -- 地面
	when airground in (0,2,3) then 0 -- 飞行
  else airground end                                             as flightStatus,    -- 飞机状态： 已启程
  spi                                                            as special,          -- 是否有特殊情况
  cast(null as string)                                           as originAirport3Code		, -- 起飞机场的iata代码
  cast(null as string)                                           as originAirportEName		, -- 来源机场英文
  cast(null as string)                                           as originAirportCName		, -- 来源机场中文
  cast(null as double)                                           as originLng					, -- 来源机场经度
  cast(null as double)                                           as originLat					, -- 来源机场纬度
  cast(null as string)                                           as destAirport3Code			, -- 目标机场的 iata 代码
  cast(null as string)                                           as destAirportEName			, -- 目的机场英文
  cast(null as string)                                           as destAirportCName			, -- 目的机场中文
  cast(null as double)                                           as destLng					, -- 目的地坐标经度
  cast(null as double)                                           as destLat					, -- 目的地坐标纬度
  cast(null as string)                                           as flightPhoto				, -- 飞机的图片
  cast(null as string)                                           as flightDepartureTime		, -- 航班起飞时间
  cast(null as string)                                           as expectedLandingTime		, -- 预计降落时间
  cast(null as double)                                           as toDestinationDistance		, -- 目的地距离
  cast(null as string)                                           as estimatedLandingDuration, -- 预计还要多久着陆
  if(CHAR_LENGTH(c.operator_icao)>10,cast(null as string),c.operator_icao) as airlinesIcao, -- 航空公司的icao代码
  c.operator                                                     as airlinesEName			, -- 航空公司英文
  c.operator_c_name                                              as airlinesCName			, -- 航空公司中文
  type                                                           as dataSource				, -- 数据来源的系统
  cast(null as string)                                           as source					, -- 来源
  country_code3,-- 经纬度位置转换国家 position_country_code2
  sea_id                                                         as seaId, -- 海域id
  h3_code                                                        as h3Code, -- 位置h3编码
  concat('{',
...
select
    a.*,
    b.country_code2 as position_country_2code,  -- 当前所处的区域
    c.c_name as seaName			  , -- 海域名字
...
from tmp_dws_aircraft_combine_list_rt_02 a
    ...
    FOR SYSTEM_TIME AS OF a.proctime as b
                   on a.country_code3 = b.country_code3
                       and 'COMMON' = b.source
...
select
    ...
    if(position_country_2code is null
        ...
        ,'CN', position_country_2code) as positionCountryCode2,
    if (coalesce(t7.country_code,t6.country_code,t5.country_code,t4.country_code) = 'CN' and registration_len = 6,
        'TW',
        coalesce(t7.country_code,t6.country_code,t5.country_code,t4.country_code)) as countryCode,    -- 去除第一位B，剩余5位就是台湾

    coalesce(t7.country_name,t6.country_name,t5.country_name,t4.country_name) as countryName  -- 国家代码
...
select
    a.*,
    b.c_name as positionCountryName					, -- 位置所在国家名称
    if(countryCode = 'TW','中国台湾',countryName) as countryName,-- 飞机所属的国家

    case
        when countryCode in ('IN','US','JP','AU','TW') and isMilitary=1 then '1' -- 敌(印度、美国、日本、澳大利亚、台湾 军事)
        when countryCode in ('CN','HK','MO') and isMilitary=1  then '2'       -- 我(中国	军事)
        when countryCode in ('CN','HK','MO') and isMilitary=0  then '3'  -- 友(中国	非军事)
        else '4' end friendFoe -- 敌我类型

from tmp_dws_aircraft_combine_list_rt_04 a
         left join dim_country_code_name_info FOR SYSTEM_TIME AS OF a.proctime as b
                   on a.positionCountryCode2 = b.country_code2
                       and 'COMMON' = b.source;
...
select
    ...
    nac_v                ,
    -- h3_code as h3_code ,
    cast(null as string) as h3_code ,
